...
public class PluginManager extends javax.swing.JFrame implements ActionListener {
  ...
  public static String prefsdir = System.getProperty("user.home")+System.getProperty("file.separator")+"CsltComm";
  ...
  private static Hashtable<String, CsltCommPlugin> pluginList = new Hashtable<String, CsltCommPlugin>();
  
  private Vector<javax.swing.JButton> buttonList;
  private ClntComm clntcomm;
  
  public PluginManager(ClntComm parent) {
    parent.loadPlugins();
    this.clntcomm = parent;
    this.buttonList = new Vector<javax.swing.JButton>();
    initComponents();
  }
  ...
  private void loadSettingsPanel(int index) {
    try{
      Vector<CsltCommPlugin> pluginList = new Vector<CsltCommPlugin>(PluginManager.getPluginList());
  ...
  private void loadIcons() {
    try{
      for(CsltCommPlugin plugin : getPluginList()) {
        BeanInfo pluginInfo = Introspector.getBeanInfo(plugin.getClass());
  ...
  public static Collection<CsltCommPlugin> getPluginList() {
    return pluginList.values();
  }
  
  /**
   * Serialize the object into an XML file to persist its values
   * @param obj The object to serialize
   */
  public static void serializeObject(Object obj) throws FileNotFoundException {
    Class objectClass = obj.getClass();
    String prefsFile = objectClass.getName()+".xml";
    
    File objectFile = new File(prefsdir, prefsFile);
    Thread.currentThread().setContextClassLoader(objectClass.getClassLoader()); //Sun BugID 4676532
  ...
  public static void getPlugins() throws MalformedURLException, ClassNotFoundException, IOException {
    ...
    //Below is my work-around for Sun BugID 4676532; this was fixed in tiger
    Thread.currentThread().setContextClassLoader(loader);
    
    for(File file : pluginfiles) {
      String currBean = file.getName();
      ...
      try { //Attempt to retrieve plugin settings from serialization file
        ...
        System.out.println("Instantiating plugin "+currBean+" from "+currBean);
        
        try {
          plugin = (CsltCommPlugin)Beans.instantiate(loader, currBean);
        } catch (ClassNotFoundException ex) { //I hate nested exceptions. Seriously.
          String workingDir = System.getProperty("user.dir");
          System.err.println("Failed loading plugin: couldn't read JavaBean from "+pluginsdir);
          continue; //We ultimately failed, so skip it
        }
      }
